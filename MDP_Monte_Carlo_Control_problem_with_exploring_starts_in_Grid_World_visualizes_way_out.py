...
@njit
def play_one_episode(coord_of_states, gridworld_ndarray, rewards_plus_qsa_values_ndarray, win_terminal_state, loose_terminal_state, starting_sate):
    """
    play one episode of the game, update Q(s,a), policy, and cum of returns for every Q(s,a)
    """
    gamma = 0.9
    state_k_coord_start_from_start_state = False
    state_k_coord = coord_of_states[random.randint(0, len(coord_of_states))] # get random state from list of all possible states
    if state_k_coord == starting_sate:
        state_k_coord_start_from_start_state = True
        # print("statrting state from play one episode, state_k_coord: ", state_k_coord)
    ...
    if len(all_actions_of_the_state) != 0: # starting state do not have any action to perform
        ...
        cycling_two_states = False
        visited_states = List()
        visited_states.append(state_k_coord)

        while (not terminal_state_bool) and (not cycling_two_states) and (action_of_state is not None):
    ...
    if state_k_coord_start_from_start_state and terminal_state_bool:
        played_from_start_state_and_reached_terminal_state = True
        print("data_of_episode_list :", data_of_episode_list)
    else:
        played_from_start_state_and_reached_terminal_state = False
    return rewards_plus_qsa_values_ndarray, gridworld_ndarray, played_from_start_state_and_reached_terminal_state
...
def play_from_random_s_a_save_returns_select_best_q_s_a (n_states_vertical, n_states_horizontal, rewards_plus_qsa_values_ndarray, gridworld_ndarray, win_terminal_state, loose_terminal_state, starting_sate):
    ...
    while played_from_start_state_and_reached_terminal_state == False:
        rewards_plus_qsa_values_ndarray, gridworld_ndarray, played_from_start_state_and_reached_terminal_state = play_one_episode(coord_of_states, gridworld_ndarray, rewards_plus_qsa_values_ndarray, win_terminal_state, loose_terminal_state, starting_sate)
        if n_of_loop%5000 == 0:
            print("Nº of loop: ", n_of_loop, " Reached terminal state: ", played_from_start_state_and_reached_terminal_state)
        n_of_loop += 1
...
def find_way_from_start_to_terminal_state(n_states_vertical, n_states_horizontal, gridworld_ndarray, starting_state, win_terminal_state):
    ...
    while not_cycling_not_terminal_state:
        best_action_for_state = get_action_from_state_and_next_state(n_states_vertical, n_states_horizontal, gridworld_ndarray_for_print, starting_state)
...
gridworld_ndarray = find_way_from_start_to_terminal_state(n_states_vertical, n_states_horizontal, gridworld_ndarray, starting_sate, win_terminal_state)
grid_world_image_ndarray = create_ndarray_grid_world_image(n_states_vertical, n_states_horizontal, image_size, images_states_ndarray, gridworld_ndarray)
